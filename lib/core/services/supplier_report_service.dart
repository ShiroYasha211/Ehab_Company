    ...
    return pw.Padding(
      ...
      child: pw.Column(
        children: [
          ...
          pw.SizedBox(height: 5),
          pw.Text(
            'تاريخ الطباعة: ${intl.DateFormat('yyyy-MM-dd').format(DateTime.now())}',
  ...
  static pw.Widget _buildTransactionsTable(
    ...
    final data = transactions.map((trans) {
      ...
      final description = trans.notes?.isNotEmpty == true
          ? trans.notes!
          : (trans.type == SupplierTransactionType.PAYMENT
                ? 'سند صرف'
                : (trans.type == SupplierTransactionType.PURCHASE
                      ? 'فاتورة شراء #${trans.referenceId}'
                      : 'رصيد افتتاحي'));

      return [
        '${intl.NumberFormat.decimalPattern('ar').format(runningBalance)}',
        debit > 0
            ? '${intl.NumberFormat.decimalPattern('ar').format(debit)}'
            : '',
        credit > 0
            ? '${intl.NumberFormat.decimalPattern('ar').format(credit)}'
            : '',
        description,
