// File: lib/core/services/printing/sales_invoice_pdf_service.dart
...
class SalesInvoicePdfService {
  static Future<void> printInvoice(Map<String, dynamic> invoiceDetails) async {
    ...
    // 2. جلب رُتب الوحدات للتفكيك الهرمي وتحويلها لخريطة
    final List<UnitModel> unitsList = await UnitRepository().getAllUnits();
    final Map<int, UnitModel> unitsMap = {for (var u in unitsList) u.id!: u};
    ...
    final String statusText = isPaid ? 'مدفوعة' : 'آجلة';
            ...
            children: [
              ...
              pw.Column(
                ...
                children: [
                  ...
                  _buildCustomerInfo(invoiceData, ttf, boldTtf),
                  pw.SizedBox(height: 15),
                  _buildItemsTable(itemsData, formatNum, ttf, boldTtf, unitsMap),
                  pw.SizedBox(height: 15),
  ...
  static pw.Widget _buildInvoiceTitle(int id, String date, pw.Font boldTtf, pw.Font ttf) {
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          PdfBaseService.safeText('فاتورة مبيعات عميل', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, font: boldTtf, color: PdfColors.indigo900)),
          pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.end,
            children: [
              PdfBaseService.safeText('رقم: #$id', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, font: boldTtf, fontSize: 11)),
  ...
  static pw.Widget _buildCustomerInfo(Map<String, dynamic> data, pw.Font ttf, pw.Font boldTtf) {
    return pw.Container(
      ...
      child: pw.Row(
        children: [
          PdfBaseService.safeText('العميل المستلم: ', style: pw.TextStyle(fontSize: 10, color: PdfColors.indigo700, font: ttf)),
          PdfBaseService.safeText(PdfBaseService.sanitize(data['customerName'] ?? 'عميل نقدي'), style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, font: boldTtf)),
        ],
      ),
    );
  }

  static pw.Widget _buildItemsTable(List<dynamic> items, String Function(double) formatNum, pw.Font ttf, pw.Font boldTtf, Map<int, UnitModel> unitsMap) {
    return pw.Table(
      ...
      children: [
        ...
        ...List.generate(items.length, (index) {
          ...
          final int? unitId = item['unitId'];
          
          // استخدام التنسيق الذكي للكميات الورقية
          final smartQty = PdfBaseService.formatSmartQuantity(unitId, qty, unitsMap);
          ...
          return pw.TableRow(
            ...
            children: [
              ...
              PdfBaseService.tableCell(formatNum((item['salePrice'] as num).toDouble()), ttf, size: 8),
              PdfBaseService.tableCell((item['freeQuantity'] as num? ?? 0).toString(), ttf, size: 8),
              PdfBaseService.tableCell(smartQty, ttf, size: 7.5), // تصغير الخط قليلاً للكميات المركبة
        ...
        pw.Expanded(
          ...
          child: (data['issuedBy'] != null) ? pw.Column(
            ...
            children: [
              ...
              PdfBaseService.safeText('توقيع المحاسب', style: pw.TextStyle(fontSize: 7, font: ttf)),
            ],
          ) : pw.SizedBox(),
        ),
        pw.Expanded(
          ...
          child: pw.Column(
            children: [
              _buildSummaryRow('الإجمالي القبل الخصم:', formatMoney(subTotal), ttf, boldTtf),
              _buildSummaryRow('إجمالي الخصم:', '- ${formatMoney(data['discountAmount'] as double)}', ttf, boldTtf, color: PdfColors.red800),
              ...
              pw.Container(
                ...
                child: _buildSummaryRow('الصافي المطلوب:', formatMoney(data['totalAmount'] as double), ttf, boldTtf, isTotal: true, color: PdfColors.indigo900),
              ),
              _buildSummaryRow('المبلغ المدفوع:', formatMoney(data['paidAmount'] as double), ttf, boldTtf, color: PdfColors.green800),
              _buildSummaryRow('المتبقي الفعلي:', formatMoney(data['remainingAmount'] as double), ttf, boldTtf, color: PdfColors.red900),
  ...
  static pw.Widget _buildPaymentsTable(List<dynamic> payments, String Function(double) formatMoney, pw.Font ttf, pw.Font boldTtf) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        PdfBaseService.safeText('سجل السداد:', style: pw.TextStyle(font: boldTtf, fontSize: 10, color: PdfColors.indigo900)),
        pw.SizedBox(height: 4),
        pw.Table(
          ...
          children: [
            pw.TableRow(
              ...
              children: [
                ...
                PdfBaseService.tableCell('التفاصيل', boldTtf, size: 8),
            ...
            ...payments.map((p) => pw.TableRow(
              children: [
                ...
                PdfBaseService.tableCell(p['method'] == 'cash' ? 'نقد (${p['fundName']})' : 'حوالة/بنك', ttf, size: 8),
