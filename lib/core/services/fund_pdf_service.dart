...
class FundPdfService {
  static Future<void> printFundReport({
    ...
    final file = File(
      '${dir.path}/fund_report_${intl.DateFormat('yyyy-MM-dd').format(from)}_to_${intl.DateFormat('yyyy-MM-dd').format(to)}.pdf',
    );
  ...
  static pw.Widget _buildTransactionsTable(
    ...
    final data = transactions.map((tx) {
      ...
      final withdrawal = tx.type == TransactionType.WITHDRAWAL
          ? tx.amount
          : 0.0;
