...
class CustomerPdfService {
  static Future<void> printCustomersReport(
    ...
    final double totalDebt = customers.fold(
      0,
      (previousValue, customer) => previousValue + customer.balance,
    );
    final int indebtedCustomersCount = customers
        .where((c) => c.balance > 0)
        .length;
      ...
      child: pw.Row(
        ...
        children: [
          ...
          _buildSummaryItem(
            ...
            indebtedCustomersCount.toDouble(),
            isCurrency: false,
          ),
        ],
      ),
  ...
  static pw.Widget _buildCustomersTable(List<CustomerModel> customers) {
    ...
    final data = customers.map((customer) {
      return [
        '${intl.NumberFormat.decimalPattern('ar').format(customer.balance)} ${Get.find<SettingsService>().primaryCurrency.value.symbol}',
