...
class TransactionDetailsScreen extends StatelessWidget {
  final dynamic
  transaction; // يمكن أن يكون إما SupplierTransactionModel أو CustomerTransactionModel
  ...
  Widget build(BuildContext context) {
    ...
    final String amount = intl.NumberFormat.currency(
    ...
    if (isSupplier) {
      partyLabel = 'المورد';
      partyName =
          (transaction as SupplierTransactionModel).supplierName ?? 'غير محدد';
      final type = (transaction as SupplierTransactionModel).type;
      transactionTypeString = type == SupplierTransactionType.PAYMENT
          ? 'سند دفع'
          : 'رصيد افتتاحي';
    } else {
      partyLabel = 'العميل';
      partyName =
          (transaction as CustomerTransactionModel).customerName ?? 'غير محدد';
      final type = (transaction as CustomerTransactionModel).type;
      transactionTypeString = type == CustomerTransactionType.RECEIPT
          ? 'سند قبض'
          : 'رصيد افتتاحي';
    }
        ...
        child: Column(
          children: [
            ...
            _buildInfoRow(
              'التأثير على الصندوق',
              affectsFund
                  ? 'نعم، تم التأثير على الصندوق'
                  : 'لا، لم يؤثر على الصندوق',
              highlightColor: affectsFund
