...
class SuppliersReportsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final SupplierReportsController controller = Get.put(
          ...
          Obx(() {
            ...
            if (controller.foundTransaction.value != null) {
              return _buildTransactionDetailsCard(
                controller.foundTransaction.value!,
              );
            }
            if (controller.noResultFound.isTrue) {
              return const Center(
                child: Text(
                  'لا يوجد سند بهذا الرقم.',
  ...
  Widget _buildSearchCard(
    ...
    SupplierReportsController controller,
  ) {
    return Card(
      elevation: 2,
      child: Padding(
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'تفاصيل السند رقم: ${transaction.id}',
            ...
            _buildInfoRow(
              isPayment ? Icons.arrow_upward : Icons.play_for_work,
              'نوع العملية:',
              isPayment ? 'دفعة نقدية' : 'رصيد افتتاحي',
            ),
            _buildInfoRow(
