...
class AddPurchaseInvoiceScreen extends StatelessWidget {
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(
                              'الكمية: ${item.quantity.toInt()} × ${item.purchasePrice.toStringAsFixed(2)} ${Get.find<SettingsService>().primaryCurrency.value.symbol}',
                            ),
                            trailing: IconButton(
                              icon: Icon(
                              ...
                              onPressed: () => controller
                                  .removeProductFromInvoice(item.product.id!),
                            ),
                            onTap: () =>
          ...
          Obx(() {
            ...
            return DropdownButtonFormField<SupplierModel>(
              value: controller.supplierController.filteredSuppliers
                  .firstWhereOrNull(
                    (s) => s.id == controller.selectedSupplier.value?.id,
                  ),
              items: controller.supplierController.filteredSuppliers.map((
                supplier,
              ) {
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText:
                        'المتوفر في المخزن: ${item.product.quantity.toInt()}',
      ...
      child: TypeAheadField<ProductModel>(
        ...
        itemBuilder: (context, suggestion) {
          return ListTile(
            ...
            trailing: Text(
              '${suggestion.purchasePrice.toStringAsFixed(2)} ${Get.find<SettingsService>().primaryCurrency.value.symbol}',
            ),
          );
        },
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                subtitle: Text(
                  '${item.quantity.toInt()} × ${item.purchasePrice.toStringAsFixed(2)}',
                  style: const TextStyle(color: Colors.grey, fontSize: 13),
                ),
                trailing: Text(
                  '${(item.quantity * item.purchasePrice).toStringAsFixed(2)} ${Get.find<SettingsService>().primaryCurrency.value.symbol}',
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 14,
                  ),
  ...
  Widget _buildPaymentSection(AddPurchaseController controller) {
    formatCurrency(double val) =>
        '${val.toStringAsFixed(2)} ${Get.find<SettingsService>().primaryCurrency.value.symbol}';
    return Obx(
      () => Column(
        children: [
          _buildDialogInfoRow(
          ...
          Container(
            ...
            child: _buildDialogInfoRow(
              'المبلغ المتبقي:',
              formatCurrency(controller.remainingAmount),
              isLarge: true,
              isBold: true,
            ),
          ),
