// File: lib/features/sales/data/services/sales_return_report_service.dart
...
class SalesReturnReportService {
  static Future<void> generateAndPreviewReport({
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          _buildReportTitle(startDate, endDate, customerName),
    ...
    await Printing.layoutPdf(
      ...
      name: 'تقرير_المرتجعات_${intl.DateFormat('yyyy_MM_dd').format(DateTime.now())}.pdf',
    );
  }
        ...
        children: [
          pw.Column(
            ...
            children: [
              ...
              pw.Text('قسم إدارة المبيعات والمرتجعات', style: const pw.TextStyle(fontSize: 10)),
            ],
          ),
  ...
  static pw.Widget _buildReportTitle(DateTime? start, DateTime? end, String? customer) {
    ...
    return pw.Center(
      child: pw.Column(
        children: [
          ...
          pw.Text('تقرير حركة المرتجعات التفصيلي', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: PdfColors.blueGrey800)),
          pw.SizedBox(height: 5),
          pw.Text('لفترة: $dateRange', style: const pw.TextStyle(fontSize: 10)),
          if (customer != null)
             pw.Text('العميل: $customer', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
        ],
      ),
    );
  }
  ...
  static pw.Widget _buildSummaryCards(List<Map<String, dynamic>> returns, String symbol) {
    ...
    return pw.Row(
      ...
      children: [
        _buildStatBox('إجمالي قيمة المرتجعات', '${totalValue.toStringAsFixed(2)} $symbol', PdfColors.blue800),
  ...
  static pw.Widget _buildReturnsTable(List<Map<String, dynamic>> returns, String symbol) {
    final headers = ['القيمة', 'السبب', 'العميل', 'التاريخ', '#'];
